...
const organisateurService = require("../services/organisateurs.services.pg");
...
exports.saveOrganisateur = async (req,res) => {
    ...
    const resultat = await organisateurService.insertOrganisateur(uuid, nom, prenom, email, password, tel);
    if (resultat) {
...
exports.getOrganisateurs = async (req, res) => {
    const organisateurs = await organisateurService.getOrganisateurs();
    if (!organisateurs) {
...
exports.updateOrganisateur = async (req,res) => {
    ...
    const password = req.body.password;
    const tel = req.body.tel;
    const resultat = await organisateurService.updateOrganisateur(uuid, nom, prenom, email, password, tel);
    if(resultat){
        return res.status(500).send("ERREUR INTERNE");
    }
...
exports.deleteOrganisateur = async (req, res) => {
    ...
    try {
        const result = await organisateurService.deleteOrganisateur(uuid);
